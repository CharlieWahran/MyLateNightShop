...
if(isset($_POST['update_qty'])){
   ...
   $update_qty = $pdo->prepare("UPDATE `cart` SET quantity =:quantity WHERE id =:id");
   $update_qty->execute([
      'quantity' => $p_qty, 
      'id' => $cart_id
   ]);
...
<section class="shopping_cart">
   ...
   <form action="" method="POST" class="box">
      ...
      <img src="uploaded_picture/<?= $fetch_cart['image']; ?>" alt="">
      ...
      <div class="sub_total"> sous total : <span><?= $sub_total = ($fetch_cart['price'] * $fetch_cart['quantity']); ?>€</span> </div>
   </form>
   ...
   <div class="cart_total">
      ...
      <a href="cart.php?delete_all" class="delete-btn <?= ($grand_total > 1)?'':'disabled'; ?>">Tout supprimer</a>
      <a href="checkout.php" class="btn <?= ($grand_total > 1)?'':'disabled'; ?>">Payer </a>
   </div>
